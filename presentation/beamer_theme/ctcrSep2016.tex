...
\begin{frame}
	...
	\begin{itemize}
		\item Creighton's paper
		\item Fuentes-Mattei's paper
		\item Cris Print
	\end{itemize}
	\vskip2ex
...
\begin{frame}{Creighton \textit{et al.} data revisited}
	Gene expression analysis was carried out on:
	\begin{itemize}
		\item Original Creighton data
		\item Creighton data (Caucasian samples only)
	\end{itemize}
	\vskip2ex
...
\begin{frame}{So \ldots BMI is not related to cancer?}
	\begin{center}
		\includegraphics[width=0.55\paperwidth]{./images/tmp/mikres}
	\end{center}
\end{frame}
...
\begin{frame}{Gatza \textit{et al.} (2010)}
	Used 18 pathways to define sub-classes of breast tumours, based on the expression of the pathway signatures
	\begin{itemize}
		\item AKT, ER, PR, HER2, PI3K, p53, MYC, RAS, etc...
	\end{itemize}
	\vskip2ex
	Each pathway signature was obtained by overexpressing a single gene in a model cell line, and then differentially expressed genes were noted down
	\begin{itemize}
			\item Use these `pathway genes' to see what the obesity-associated genes are most similar to
	\end{itemize}
\end{frame}
...
\begin{frame}{Results}
	...
	\begin{itemize}
		\item BMI-only
		\item BMI and pathway metagenes
		\item Pathways-only
